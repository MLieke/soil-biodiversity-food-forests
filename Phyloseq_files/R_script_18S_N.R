#Dada2 pipeline Feb-2024 for 18S_N data
...
setwd("/home/nioo/isabellez/sequencing/18S_N/18S_N_4R_NemaTaxa")

path <- "/home/nioo/isabellez/sequencing/18S_N/18S_N_4R_NemaTaxa/rawdata"  ## CHANGE ME to the directory containing the fastq files.
list.files(path)
...
FWD <- "GGTGGTGCATGGCCGTTCTTAGTT"  ## NF1
REV <- "TACAAAGGGCAGGGACGTAAT"  ## 18Sr2b
...
# FWD.ForwardReads   15422          0       0       0
# FWD.ReverseReads       0          0       0       1
# REV.ForwardReads       0          0       0       0
# REV.ReverseReads   15347          0       0       0
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = c(2,2), 
                     truncQ = 2, rm.phix = TRUE, compress = TRUE, matchIDs = TRUE, truncLen=c(220,210), multithread=40)  # on windows, set multithread = FALSE
head(out)
...
#Assign taxonomy
NemaTaxa_ref <- "/home/nioo/isabellez/sequencing/18S_N/NemaTaxa/NemaTaxa_V1.train.fasta"
taxa <- assignTaxonomy(seqtab.nochim, NemaTaxa_ref, multithread = 40, tryRC = TRUE)
...
vb_code <- read.csv2("dna_code_18S_N_site.csv")
vb_code <- vb_code %>% mutate_at("dna_code", str_replace, "N", "")
vb_code$dna_code <- as.integer(vb_code$dna_code)
samdf <- data.frame(Sample=samples.out)
samdf <- samdf %>% separate(Sample, c("i5","dna_code"))
samdf <- samdf %>% mutate_at("dna_code", str_replace, "N", "")
...
ps <- phyloseq(otu_table(seqtab.nochim, taxa_are_rows=FALSE), sample_data(samdf),
               tax_table(taxa))
saveRDS(ps, "ps_18S_N_NemaTaxa.rds")