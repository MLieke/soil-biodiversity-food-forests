#Dada2 pipeline Feb-2024 for 16S data
...
###### Dada2 pipeline for bacteria and archaea
...
setwd("/home/nioo/isabellez/sequencing/16S/16S_nextseq_4R")

path <- "/home/nioo/isabellez/sequencing/16S/16S_nextseq_4R/rawdata"  ## CHANGE ME to the directory containing the fastq files.
list.files(path)
...
REV <- "CCGYCAATTYMTTTRAGTTT"  ## CHANGE ME...
...
# FWD.ForwardReads   47751          0       0       0
# FWD.ReverseReads       0          0       0       7
# REV.ForwardReads       0          0       0       5
# REV.ReverseReads   44945          0       0       0
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = c(2, 2), 
                     truncQ = 2, rm.phix = TRUE, compress = TRUE, matchIDs = TRUE, truncLen=c(250,220), multithread=40)  # on windows, set multithread = FALSE
head(out)
...
# gtdb_ref <- "/home/nioo/isabellez/sequencing/16S/gtdb_database/GTDB_bac120_arc53_ssu_r214_fullTaxo.fa.gz"  # CHANGE ME to location on your machine
silva_ref <- "/data/shared/databases/silva/dada2/138.1/silva_nr99_v138.1_train_set.fa.gz"
set.seed(19203)
taxa <- assignTaxonomy(seqtab.nochim, silva_ref, multithread = 40, tryRC = TRUE)

# #Assign taxonomy AND 100% match species annotation DOESNT WORK YET?!
# silva_ref_sp <- "/data/shared/databases/silva/dada2/138.1/silva_nr99_v138.1_wSpecies_train_set.fa.gz"
# taxa_sp <- addSpecies(taxa, silva_ref_sp, verbose=TRUE)
...
write.table(asv_tab, "ASVs_counts_genus.csv", sep="\t", quote=F, col.names=NA)

# tax table
asv_tax <- taxa
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "ASVs_taxonomy_genus.csv", sep="\t", quote=F, col.names=NA)

# Concatenate Counts and Taxonomy tables
counts <- read.table("ASVs_counts_genus.csv", header = TRUE, sep = "\t", dec = ".", row.names = 1)
taxonomy <- read.table("ASVs_taxonomy_genus.csv", header = TRUE, sep = "\t", dec = ".", row.names = 1)
counts_taxonomy <- as.data.frame(c(taxonomy, counts))
write.table(counts_taxonomy, "ASVs_tax_counts_genus.csv", sep="\t", quote=F, col.names=NA)
...
vb_code <- read.csv2("dna_code_16S_site.csv")
