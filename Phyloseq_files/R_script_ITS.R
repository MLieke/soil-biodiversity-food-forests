#Dada2 pipeline Feb-2024 for ITS data
...
library(tidyverse)
library(magrittr) #for this learnError function from the forum
library(phyloseq)
library(ggvenn)
library(MicEco)
library(vegan)
...
setwd("/home/nioo/isabellez/sequencing/ITS/ITS_4R")

path <- "/home/nioo/isabellez/sequencing/ITS/ITS_4R/rawdata"  ## CHANGE ME to the directory containing the fastq files.
list.files(path)
...
FWD <- "GTGARTCATCGARTCTTTG"  ## CHANGE ME to your forward primer sequence
...
# Forward Complement Reverse RevComp
# FWD.ForwardReads   75789          0       0       0
# FWD.ReverseReads       0          0       0   38732
# REV.ForwardReads       0          0       0   38349
# REV.ReverseReads   75703          0       0       0
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = c(2, 2), 
                     truncQ = 2, minLen = 50, rm.phix = TRUE, compress = TRUE, matchIDs = TRUE, multithread=40)  # on windows, set multithread = FALSE
head(out)


#Solving the problem with Learn Error argument (due the Next/NovoSeq platform)
...
# from this forum: https://github.com/ErnakovichLab/dada2_ernakovichlab#learn-the-error-rates
...
dim(seqtab.nochim)
sum(seqtab.nochim)/sum(seqtab) #closer to 1, better! 0.9895006
...
#Assign taxonomy
unite_ref <- "/data/shared/databases/unite/dada2/2023-07-18_version-9.0/sh_general_release_dynamic_18.07.2023.fasta"
taxa <- assignTaxonomy(seqtab.nochim, unite_ref, multithread = 40, tryRC = TRUE)
...
vb_code <- read.csv2("dna_code_ITS_site.csv")
