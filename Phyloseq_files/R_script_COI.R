#Dada2 pipeline Feb-2024 for COI primers
...
library(ggvenn)
library(MicEco)
packageVersion("MicEco")

#Setup the work folder
setwd("/home/nioo/isabellez/sequencing/COI/COI_ASV_BOLDigger2")

path <- "/home/nioo/isabellez/sequencing/COI/COI_4R/rawdata"  ## CHANGE ME to the directory containing the fastq files.
list.files(path)
...
REV <- "TANACYTCNGGRTGNCCRAARAAYCA"  ## jgHCO2198 NOTE! Had to change I to N
...
# FWD.ForwardReads   90347          0       0       0
# FWD.ReverseReads       2          0       0    1352
# REV.ForwardReads       2          0       0    1255
# REV.ReverseReads   88272          0       0       0
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = c(2,2),
                     truncQ = 2, rm.phix = TRUE, compress = TRUE, matchIDs = TRUE,
                     truncLen=c(210,200), multithread=40)  # on windows, set multithread = FALSE
head(out)
...
# # Classic error function from DADA2 tutorial
#
# errF <- learnErrors(filtFs, multithread=40)
# errR <- learnErrors(filtRs, multithread=40)
# plotErrors(errF, nominalQ = TRUE)

# I skiped the Reverse Learn Error
#I will add the Learn Error fom Forwards to the Reverse
...
#make fasta for vsearch
seqtab.nochim <- readRDS("seqtab.nochim.rds")
sq <- getSequences(seqtab.nochim)
abundances <- colSums(seqtab.nochim)
id_abund <- paste0("ASV", seq_len(length(sq)), ";size=", abundances, ";")
seq_id <- data.frame(sq,id_abund)
names(sq) <- id_abund
writeFasta(sq, "seqtab_abund.fasta")

#load OTUs from vsearch
fastaFile <- readDNAStringSet("output_abund.fasta")
seq_name = names(fastaFile)
sequence = paste(fastaFile)
OTUs <- data.frame(seq_name, sequence)

# Load the uc file
uc_data <- read.table("output_clusters.uc", header = FALSE, stringsAsFactors = FALSE)
# Extract relevant columns: Column 9 is the original sequence (ASV), Column 10 is the centroid
asv_to_centroid <- uc_data[uc_data$V1 == "H", c(9, 10)]
colnames(asv_to_centroid) <- c("ASV", "Centroid")
# For centroids themselves, we can assign the centroid sequence as its own representative
centroids <- uc_data[uc_data$V1 == "S", c(9,9)]
colnames(centroids) <- c("ASV", "Centroid")
# Combine hits and centroids into a single mapping table
asv_to_centroid <- rbind(asv_to_centroid, centroids)

setwd("/home/nioo/isabellez/sequencing/COI/COI_OTU_taxalogue")
seqtab.nochim <- readRDS("seqtab.nochim.rds")

#make clustered OTU abundance matrix
asv_abundances <- as.data.frame(t(seqtab.nochim))
id_abund <- paste0("ASV", seq_len(length(sq)), ";size=", abundances, ";")
asv_abundances$id <- id_abund
asv_abundances$Centroid <- asv_to_centroid$Centroid[match(asv_abundances$id, asv_to_centroid$ASV)]
centroid_abundances <- asv_abundances %>% select(-"id") %>%
  group_by(Centroid) %>%
  summarise(across(everything(), sum, na.rm = TRUE))
centroid_abundances <- centroid_abundances %>% left_join(seq_id, join_by(Centroid==id_abund))
centroid_abundances <- as.data.frame(centroid_abundances)
rownames(centroid_abundances) <- centroid_abundances$sq
centroid_abundances <- subset(centroid_abundances, select=-c(Centroid,sq))
centroid_abundances <- t(centroid_abundances)
saveRDS(centroid_abundances, "centroid_abundances.RDS")

# #manually merge 97% similar ASVs
...
# clusters <- DECIPHER::IdClusters( #Error: 'IdClusters' is not an exported object from 'namespace:DECIPHER'
#   d,
#   method = "complete",
...
centroid_abundances <- readRDS("centroid_abundances.RDS")

# #Assign taxonomy
# midori <- "/home/nioo/isabellez/sequencing/COI/MIDORI2/MIDORI2_UNIQ_NUC_GB261_CO1_DADA2.fasta.gz"
# taxa <- assignTaxonomy(seqtab.nochim, midori, multithread = 40, tryRC = TRUE)

# Assign taxonomy
# bold <- "/home/nioo/isabellez/sequencing/COI/BOLD/bold_clustered.assigntaxonomy.fasta.gz"
bold <- "/home/nioo/isabellez/sequencing/COI/BOLDv4/BOLD_Public.27-Sep-2024.fasta.gz"
taxalogue <- "/scratch/mattiash/BOLD/taxalogue/results/Arthropoda-20241007T1148/merged_dada2_taxonomy_nospecies.fas"
taxa <- assignTaxonomy(centroid_abundances, taxalogue, multithread = 40, tryRC = TRUE)
species <- "/scratch/mattiash/BOLD/taxalogue/results/Arthropoda-20241007T1148/merged_dada2_species.fas"
taxa_sp <- addSpecies(taxa, species, tryRC = TRUE)
...
#Adjusting the names
asv_seqs <- colnames(centroid_abundances)
asv_headers <- vector(dim(centroid_abundances)[2], mode="character")
for (i in 1:dim(centroid_abundances)[2]) {
...
# FOR taxa_sp
#Saving the taxa file
saveRDS(taxa_sp, "taxa_sp.rds")
write.table(taxa_sp, "taxa_sp.csv", sep="\t", quote=F)

# tax table
asv_tax_sp <- taxa_sp
row.names(asv_tax_sp) <- sub(">", "", asv_headers)
write.table(asv_tax_sp, "ASVs_taxonomy_sp.csv", sep="\t", quote=F, col.names=NA)

# Concatenate Counts and Taxonomy tables
taxonomy_sp <- read.table("ASVs_taxonomy_sp.csv", header = TRUE, sep = "\t", dec = ".", row.names = 1)
counts_taxonomy_sp <- as.data.frame(c(taxonomy_sp, counts))
write.table(counts_taxonomy_sp, "ASVs_tax_counts_sp.csv", sep="\t", quote=F, col.names=NA)
...
# NAs
NAS <- taxonomy %>% summarise(na_yes = sum(is.na(Genus)),
          na_no = sum(!is.na(Genus)))
NAS$frac_yes <- NAS$na_yes/(NAS$na_yes + NAS$na_no)
NAS$frac_no <- NAS$na_no/(NAS$na_yes + NAS$na_no)
NAS

#sample codes
samples.out <- rownames(centroid_abundances)
vb_code <- read.csv2("dna_codes_COI.csv")[,-7]
samdf <- data.frame(dna_code=samples.out)
samdf$dna_code <- as.character(samdf$dna_code)
samdf <- left_join(samdf,vb_code,by="dna_code")

rownames(samdf) <- samples.out
ps_COI_OTU <- phyloseq(otu_table(centroid_abundances, taxa_are_rows=FALSE), sample_data(samdf),
               tax_table(taxa))
saveRDS(ps_COI_OTU, "ps_COI_OTU.rds")

#taxa species ps
ps_COI_OTU_sp <- phyloseq(otu_table(centroid_abundances, taxa_are_rows=FALSE), sample_data(samdf),
                       tax_table(taxa_sp))
saveRDS(ps_COI_OTU_sp, "ps_COI_OTU_sp.rds")