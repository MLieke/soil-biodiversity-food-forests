...
for(m in names(diversitylist_D0)){
  ...
  p <- ggplot_the_response(
    ...
    fit_pairs = boot_contrasts,
      ...
      y_label = "Species/taxon richness",
      g_label = "none"
    ) +
...
for(m in names(diversitylist_D1)){
  ...
  if(m %in% c("isopods", "millipedes", "centipedes", 
              "harvestmen", "carabids", "earthworms")){
    fitted_model <- glmmTMB(D1 ~ landuse + (1|location),
                            family = tweedie,
                            data = df)}
  ...
  extra_disp <- if(!m %in% c("protists", "mites")) {sqrt(sum(insight::get_variance(fitted_model)[["var.intercept"]]))} else 0
  
  # put model summary into dataframe
  if(!(m %in% c("protists", "nematodes", "mites"))){ # for these groups did it separately, manually, was quicker here
    reptable <- as.data.frame(report_table(fitted_model))
    modeloutput <- data.frame(group = m,
                      dispersion = reptable %>% 
  ...
  p <- ggplot_the_response(
    ...
    fit_pairs = boot_contrasts,
      ...
      contrast_rows = "all",
      palette = pal_okabe_ito_FF,
      y_label = "Effective number of species/taxa",
      g_label = "none"
    ) +
      theme(axis.text.x = element_blank(),
            axis.title.y = element_text(size = 18),
  ...
  # for springtails, mites, millipedes and harvestmen define y-axis breaks
  if(m == "spingtails"){
    p <- p +
      scale_y_continuous(breaks = c(0,2,4,6,8,10,12))
  }
  
  if(m %in% c("mites", "millipedes", "harvestmen")){
    p <- p +
...
# produce complete table with ratios and their CIs
write.csv(diversityratios, "Output_tables/H2.ratios_D1.csv")
