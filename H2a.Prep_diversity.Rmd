...
```{r data_seq}
# read dataframes with alpha-diversity at the plot level
...
harvestmen <- read.csv("Data/Harvestmen.csv") %>%
  select(!c(Stage, Sex)) %>%
  # for the diversity analyses remove _spec. entries except for Leiobunum spec.
  filter(!(species %in% c("NEMASTOMA_SPEC.", 
                          "NEMASTOMATIDAE_SPEC.",  "PHALANGIIDAE_SPEC."))) %>%
  as.data.frame()
